download.CRUNCEP <- function(years,
                             lat,lon,in.prefix = "CRUNCEP",
                             outfolder){
  ...
  for (i in seq(1,length(years))){
    ...
    for (j in seq(1,length(var$DAP.name))){
      ...
      loc.file <- file.path(outfolder, paste(in.prefix, year,
                                             "nc", sep = "."))
...
# scp /home/femeunier/Documents/projects/SecondFor/scripts/Download.CRUNCEP.R hpc:/kyukon/data/gent/vo/000/gvo00074/felicien/R
